...
public class CableServlet extends HttpServlet  {
		...
		try {
			...
			if( req.getParameter("action3")!= null){
				...
				//	String S_capacite = req.getParameter("S_capacite");
					consommable i=new consommable(1L, S_nom,"Cable",S_quantite, S_marque, S_modele, S_longeur,S_debit,"#############");
	...
	//	String S_capacite = req.getParameter("S_capacite");
			try {
				op.add(new consommable(1L, S_nom,"Cable" ,S_quantite, S_marque, S_modele, S_longeur,S_debit,"#############"));
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
